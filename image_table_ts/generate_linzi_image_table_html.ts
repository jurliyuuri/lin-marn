...
function generate_table_narrow_html(id?: string, hidden_columns?: string[]): string {
	...
	ans += imageAuthors
		.map((author) => {
			const count_from_this_author = linzi_list.filter((linzi) =>
				folder_names.filter(fname => folder_type[fname] === author).filter(name =>
	...
	linzi_list.map((linzi) => {
		...
		ans += imageAuthors.map((author, index) =>
			...
			+ (hidden_columns?.includes(author) || hidden_columns?.includes(`${index}`) ? `<span style="color: #005242">(hidden)</span>` :
